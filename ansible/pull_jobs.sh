...
# ARG_OPTIONAL_REPEATED([job],[j],[Jobs to pull - use multiple times if needed. Pass none to auto-detect all job configuration on the server and pull all present files.])
# ARG_OPTIONAL_BOOLEAN([verbose],[v])
# ARG_HELP([Pulls and renames server-side jobs configurations from the OpenSCAP Jenkins master server.])
# ARG_DEFAULTS_POS([])
# DEFINE_SCRIPT_DIR([])
# ARGBASH_GO()
...
print_help ()
{
	printf '%s\n' "Pulls and renames server-side jobs configurations from the OpenSCAP Jenkins master server."
	...
	printf '\t%s\n' "-j,--job: Jobs to pull - use multiple times if needed. Pass none to auto-detect all job configuration on the server and pull all present files. (empty by default)"
	printf '\t%s\n' "-h,--help: Prints help"
}
